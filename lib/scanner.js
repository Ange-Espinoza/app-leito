import { DIAS, leerTurno, normalizar, primerNombre } from "./schedule";
...
- Cuida de no correrte de fila: el nombre y el turno tienen que ser de la misma línea de la tabla.`;
  ...
  const texto = await preguntarProxy(base64, instruccion);
  const miNombre = normalizar(primerNombre(ficha.nombre));
  const miTurno = leerTurno(d.turno);
  return texto.split("\n")
    .map((l) => l.trim())
    .filter((l) => l.includes("|"))
    .map((l) => {
    ...
    .filter((p) => {
      // Excludes Leito re-appearing as his own "coworker": the two OCR passes
      // (the full-row search vs. this per-day column read) sometimes spell his
      // surname slightly differently ("Urra" vs "Urrea"), so a strict
      // full-name match misses it. First name + his own exact shift time for
      // this day is a much more reliable signal that it's the same row.
      const t = leerTurno(p.turno);
      const mismoNombre = normalizar(primerNombre(p.nombre)) === miNombre;
      const mismoTurno = !!(miTurno && miTurno.ok && t.s === miTurno.s && t.e === miTurno.e);
      return !(mismoNombre && mismoTurno);
    })
    .map((p) => {
      const t = leerTurno(p.turno);
