...
- Cuida de no correrte de fila: el nombre y el turno tienen que ser de la misma línea de la tabla.`;
  ...
  return texto.split("\n")
    ...
    .filter((p) => {
      // Excludes Leito re-appearing as his own "coworker": the per-day column
      // re-read sometimes spells his surname differently than the full-row
      // search did ("Urra" vs "Urrea"), so a full-name match misses it. He's
      // the only person with his first name at this store, so first-name
      // alone is enough to recognize the row is his, regardless of how the
      // rest got OCR'd.
      return normalizar(primerNombre(p.nombre)) !== miNombre;
    })
    .map((p) => {
      const t = leerTurno(p.turno);
